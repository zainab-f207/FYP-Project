"""Verify ALL locations from fir_summary.txt have geocoding coverage."""
...
def translate_structured(name):
    # Strip sub-block suffix
    base_name = re.sub(r'\s+\u0633\u0628\s+\u0628\u0644\u0627\u06a9\s+\S+$', '', name).strip()
    
    patterns = [
        (re.compile(r'\u0688\u06cc\s*\u0627\u06cc\u0686\s*\u0627\u06d2\s*\u0641\u06cc\u0632\s*(\d+)\s*\u0648\u0627[\u0626]?\u06cc\s*\u0628\u0644\u0627\u06a9'), lambda m: f"DHA Phase {m.group(1)} Y Block"),
        ...
        # PIA Society Block X
        (re.compile(r'\u067e\u06cc\s*\u0622\u0626\u06cc\s*\u0627\u06d2\s*\u0633\u0648\u0633\u0627\u0626\u0679\u06cc\s*\u0628\u0644\u0627\u06a9\s*(\S+)'), lambda m: f"PIA Society Block {m.group(1)}"),
        # Lake City Sector MX
        (re.compile(r'\u0644\u06cc\u06a9\s*\u0633\u0679\u06cc\s*\u0633\u06cc\u06a9\u0679\u0631\s*(\S+)'), lambda m: f"Lake City Sector {m.group(1)}"),
...
for loc in sorted(locations):
    ...
    # 1. Direct GEOCODE_MAPPINGS lookup
    if loc in geocode_mappings or loc_nfc in normalized_mappings:
        continue
    
    # 2. Structured translator (handles sub-block stripping internally)
    if translate_structured(loc) or translate_structured(loc_nfc):
        continue
    
    # 3. Sub-block stripping + regular lookup
    base = re.sub(r'\s+\u0633\u0628\s+\u0628\u0644\u0627\u06a9\s+\S+$', '', loc).strip()
    base_nfc = unicodedata.normalize('NFC', base)
    if base != loc:
        if base in geocode_mappings or base_nfc in normalized_mappings:
            continue
    
    # 4. Parenthetical fallback
    paren_match = re.match(r'^(.+?)\s*\(', loc)
    if paren_match:
        pbase = paren_match.group(1).strip()
        pbase_nfc = unicodedata.normalize('NFC', pbase)
        if pbase in geocode_mappings or pbase_nfc in normalized_mappings:
            continue
    
    unmapped.append(loc)

print(f"Total unique locations: {len(locations)}")
print(f"Mapped: {len(locations) - len(unmapped)}")
