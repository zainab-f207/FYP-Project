"""Write unmapped locations to unmapped.txt for inspection."""
import re, os
...
unmapped = [loc for loc in sorted(locations) if loc not in geocode_mappings and not translate_structured(loc)]
with open('unmapped.txt', 'w', encoding='utf-8') as f:
    for u in unmapped:
        f.write(u + '\n')
print(f"Unmapped: {len(unmapped)}")
print(f"Total: {len(locations)}, Mapped: {len(locations) - len(unmapped)}")
