...
def geocode_crime_area(area_name: str, city: str = "Lahore") -> dict:
    ...
    def translate_structured(name):
        ...
        patterns = [
            ...
            # پی سی ایس آئی آر فیز X بلاک Y → PCSIR Phase X Block Y
            (_re.compile(r'پی\s*سی\s*ایس\s*آئی\s*آر\s*فیز\s*(\d+)\s*بلاک\s*(\S+)'), lambda m: f"PCSIR Phase {m.group(1)} Block {m.group(2)}"),
    ...
    try:
        ...
        # 5. If name has parentheses, try base name without parenthetical
        import re as _re2
        paren_match = _re2.match(r'^(.+?)\s*\(', area_name)
        if paren_match:
            base_name = paren_match.group(1).strip()
            base_english = GEOCODE_MAPPINGS.get(base_name) or normalized_mappings.get(unicodedata.normalize('NFC', base_name))
            if base_english:
                queries.append(f"{base_english}, {city}, Pakistan")
            ...
            try:
                ...
                if hasattr(location, 'latitude') and hasattr(location, 'longitude'):
                    ...
                    if 31.0 <= lat <= 32.0 and 73.5 <= lon <= 75.0:
                        ...
                        logger.warning(f"[Geocode] Result outside Lahore area: ({lat}, {lon})")
        ...
        logger.warning(f"[Geocode] ✗ No Nominatim results for '{area_name}', trying hardcoded fallback")
        
        # Hardcoded coordinate fallback for areas Nominatim doesn't cover well
        HARDCODED_COORDS = {
            # Bahria Town sectors (large development, Nominatim often misses sectors)
            "Bahria Town": (31.3680, 74.1800),
            "Bahria Orchard": (31.3550, 74.1650),
            # Askari (military housing)
            ...
            "DHA": (31.4700, 74.4000),
            # PIA Society
            "PIA Society": (31.4950, 74.3700),
            "PIA Society Block A": (31.4960, 74.3680),
            "PIA Society Block B": (31.4950, 74.3690),
            "PIA Society Block C": (31.4940, 74.3700),
            "PIA Society Block D": (31.4930, 74.3710),
            "PIA Society Block E": (31.4920, 74.3720),
            "PIA Society Block F": (31.4910, 74.3730),
            "PIA Society Block G": (31.4900, 74.3740),
            "PIA Society Block H": (31.4890, 74.3750),
            "PIA Society Block I": (31.4880, 74.3760),
            "PIA Society Block J": (31.4870, 74.3770),
            "PIA Society Block K": (31.4860, 74.3780),
            # Lake City
            "Lake City": (31.3650, 74.1450),
            "Lake City Sector M1": (31.3660, 74.1440),
            "Lake City Sector M2": (31.3650, 74.1450),
            "Lake City Sector M3": (31.3640, 74.1460),
            "Lake City Sector M4": (31.3630, 74.1470),
            "Lake City Sector M5": (31.3620, 74.1480),
            "Lake City Sector M7": (31.3610, 74.1490),
            "Lake City Sector M8": (31.3600, 74.1500),
            # Cavalry Ground
            "Cavalry Ground": (31.5200, 74.3600),
            "Cavalry Ground Lahore": (31.5200, 74.3600),
            # Other new locations
            "Sarfraz Road Cantt Lahore": (31.5100, 74.3600),
            "Singhpura Pul Lahore": (31.5740, 74.3100),
            "Fauji Colony Cantt Lahore": (31.5070, 74.3500),
            "Fortress Stadium Lahore": (31.5180, 74.3560),
            "Fortress Stadium": (31.5180, 74.3560),
            "Lal Kurti Cantt Lahore": (31.5050, 74.3550),
            "Nawan Kot Lahore": (31.5420, 74.3080),
            "Nawan Kot": (31.5420, 74.3080),
            "Chauburji Underpass Lahore": (31.5450, 74.3350),
            "Chauburji": (31.5450, 74.3350),
            "Kamahan Interchange Lahore": (31.5600, 74.2900),
            "Kamahan Interchange": (31.5600, 74.2900),
            "Gajjumata Mor Lahore": (31.4500, 74.2400),
            "Gajjumata": (31.4500, 74.2400),
            # General Lahore center
            "Press Club Quetta": (30.1950, 67.0100),
        }
        
        # Try exact match, then partial match
        eng_name = english or structured_english or area_name
        if eng_name in HARDCODED_COORDS:
            lat, lon = HARDCODED_COORDS[eng_name]
            return {'latitude': lat, 'longitude': lon, 'display_name': f"{eng_name}, Lahore", 'success': True}
        
        # Try parent area match (e.g., "Bahria Town Sector C Block C" -> "Bahria Town")
        for key in sorted(HARDCODED_COORDS.keys(), key=len, reverse=True):
            if eng_name and key in eng_name:
                lat, lon = HARDCODED_COORDS[key]
                return {'latitude': lat, 'longitude': lon, 'display_name': f"{eng_name}, Lahore", 'success': True}
...
class FIRExtractor:
    ...
    def _apply_ocr_corrections(self, sections: set) -> set:
        ...
        for section in sections:
            ...
            if section in corrections:
                ...
                if not has_341 and corrected == '341':
                    result.add(corrected)
                    logger.info(f"  Correcting {section} → {corrected}")
                else:
                    logger.info(f"  Removing {section} (corrected version {corrected} exists)")
            else:
                ...
                if section in common_sections:
                    ...
                    try:
                        num = int(section)
                        # Only keep if in the common PPC range
                        if 100 <= num <= 520:
                            result.add(section)
                        else:
    ...
    def _parse_sections_from_text(self, text: str) -> List[str]:
        ...
        compound_section_pattern = r'(\d{3})\s*/\s*(\d{3})'
        ...
        logger.info(f"[PURE OCR] Sections with Urdu markers: {sections_with_marker}")
        ...
        # 10. Extract standalone 3-digit numbers (more restrictive)
        # Only accept if surrounded by separators (comma, space, newline, etc.)
        ...
        for num in standalone_matches:
            try:
                ...
                if num in ppc_sections:
                    continue
                # Only accept common PPC section ranges OR if already found by other methods
                # Valid PPC sections are typically 100-600 range
                if 100 <= num_int <= 600 and num not in ppc_sections:
                    ppc_sections.append(num)
                    logger.info(f"[PURE OCR] ✓ Standalone section: {num}")
                elif num_int > 600:
                    # Higher numbers (700+, 800+, 900+) are often false positives
                    logger.info(f"[PURE OCR] ✗ Skipping high number (likely noise): {num}")
            except ValueError:
                continue
    ...
    def extract_fir_data(self, image_bytes: bytes, filename: str = "") -> Dict:
        ...
        try:
            ...
            location_info = {
                ...
                'name_source': 'hash_lookup' if hash_result else 'ocr'
            }
