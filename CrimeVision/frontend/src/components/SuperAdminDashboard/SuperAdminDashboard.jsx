...
const SuperAdminDashboard = () => {
  ...
  // Close notifications popup on outside click / Esc. The popup is portalled
  // to document.body so it's not inside notifRef — check both refs.
  useEffect(() => {
    if (!notifOpen) return;
    const onDoc = (e) => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Switched from getUserAlerts to getAdminNotifications so the bell
      // surfaces audit-log activity, pending approvals and other ops events
      // generated by the role-aware /admin/notifications endpoint.
      ...
      // Transform /admin/notifications payload into the shape this dashboard
      // already uses. Each entry has {id, type, title, message, timestamp}.
      const transformedNotifications = (alertsData || []).slice(0, 12).map((n, index) => ({
        id: n.id || index,
        type: n.type || 'info',
        message: n.title ? `${n.title} — ${n.message || ''}`.trim().replace(/—\s*$/, '').trim() : (n.message || 'System notification'),
        time: formatTimeAgo(n.timestamp || n.created_at),
      ...
      >
        ...
        <nav className={styles.sidebarNav} data-sv-sidebar-nav>
          <ul className={styles.sidebarMenu} data-sv-sidebar-menu>
            ...
            <li className={styles.animateIn} style={{ transitionDelay: '650ms' }}>
              <a
                href="#audit-logs"
                className={activeSection === 'audit-logs' ? styles.active : ''}
                onClick={() => handleSectionChange('audit-logs')}
                aria-current={activeSection === 'audit-logs' ? 'page' : undefined}
              >
                <i className="fas fa-clipboard-list" style={{ fontSize: '18px', width: '20px', textAlign: 'center' }}></i>
      ...
      >
        ...
        <header className={styles.topNavbar} data-sv-navbar>
          ...
          <div className={styles.navRight}>
            ...
            {/* Portal the popup so the navbar's backdrop-filter cannot
                trap a position:fixed/absolute descendant. */}
            {notifOpen && createPortal(
              ...
              >
                <div className={styles.notifPopupHead}>
                  <strong>Notifications</strong>
                  <span>{notifications.length}</span>
                </div>
                <div className={styles.notifPopupList}>
                  {notifications.length === 0 ? (
                    <div className={styles.notifPopupEmpty}>
                      <i className="fas fa-check-circle"></i>
                      <span>You're all caught up</span>
                    </div>
                  ) : (
                    notifications.map((n) => (
                      <div key={n.id} className={`${styles.notifPopupRow} ${styles[`notifTone_${n.type || 'info'}`] || ''}`}>
                        <i className={
                          n.type === 'error' ? 'fas fa-exclamation-triangle'
                            : n.type === 'warning' ? 'fas fa-exclamation-circle'
                            : n.type === 'success' ? 'fas fa-check-circle'
                            : 'fas fa-info-circle'
                        }></i>
                        <div className={styles.notifPopupBody}>
                          <div className={styles.notifPopupMsg}>{n.message}</div>
                          <span className={styles.notifPopupTime}>{n.time}</span>
                        </div>
                      </div>
                    ))
                  )}
                ...
                <button
                  className={styles.notifPopupFoot}
