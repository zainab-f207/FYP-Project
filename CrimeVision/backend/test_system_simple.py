#!/usr/bin/env python3
"""
Simple System Status Check - No Emojis
"""
import requests
import json
import os
    ...
    try:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print("SUCCESS: VAPID endpoint working")
                ...
                if public_key.startswith(expected_start):
                    print("SUCCESS: Using original VAPID keys (should fix browser notifications)")
                else:
                    print("WARNING: Using different VAPID keys")
            else:
                print(f"ERROR: VAPID endpoint failed: {response.status_code}")
        except Exception as e:
            print(f"ERROR: VAPID endpoint error: {e}")
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                print("SUCCESS: getUserStats endpoint working")
                ...
                if time_filter == 'all':
                    print("SUCCESS: Time filter correctly returned as 'all'")
                else:
                    print(f"ERROR: Time filter should be 'all' but got '{time_filter}'")

                if total_crimes > 0:
                    print(f"SUCCESS: Found {total_crimes} crimes (dashboard should work)")
                else:
                    print("ERROR: No crimes found - backend SQL fixes not loaded")

            else:
                print(f"ERROR: getUserStats failed: {response.status_code}")
                print(f"Response: {response.text[:200]}...")

        except Exception as e:
            print(f"ERROR: getUserStats error: {e}")
        ...
        print("")
        print("REQUIRED ACTION: Restart backend server!")
